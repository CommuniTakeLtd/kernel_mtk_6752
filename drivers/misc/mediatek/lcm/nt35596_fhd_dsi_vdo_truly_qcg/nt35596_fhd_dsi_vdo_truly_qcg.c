...
//add by zhangyun for tzd lcm@{
#define LCM_ID_NT35596_TZD (0x9E)
static unsigned int lcm_qcg_tzd = 0;
//@}
#ifndef TRUE
    #define TRUE 1
#endif
...
static void lcm_init(void)
{
	...
	MDELAY(20);  
        //add by zhangyun for tzd lcm @{
        unsigned char buffer_tzd[2];
        unsigned int id_tzd=0;
        //@}
      //add by zhangyun for tzd lcm @{
        read_reg_v2(0x04, buffer_tzd, 2);
        id_tzd = buffer_tzd[0]; //tzd lcm special id use to distinguish from before nt35596 ic lcm
        if(id_tzd == LCM_ID_NT35596_TZD)
           {
            lcm_qcg_tzd=1;
           }
        //@}
//change by zhangyun for auto select init by id 
	if(lcm_qcg_tzd == 0)
	{
...
static unsigned int lcm_compare_id(void)
{
	...
	// zhangyun add new lcm @{
	if(buffer_1[0] == 0x00000000 && buffer_2[0] == 0x00000080 && buffer_3[0] == 0x00000000)
	{
	#ifdef BUILD_LK
		printf("%s, new lk nt35596 QCG horse debug: lcm_qcg = %d\n", __func__,lcm_qcg);
	#else
		printk("%s, new kernel nt35596 QCG horse debug: lcm_qcg = %d\n", __func__,lcm_qcg);
	#endif
		lcm_qcg = 1;
	}
	//end @}
  //change by zhangyun for tzd lcm
    if(id == LCM_ID_NT35596 && lcm_qcg == 1||id == LCM_ID_NT35596 && lcm_qcg_tzd == 1)
    	return 1;
    else
        return 0;
