...
{
    pr_debug( "[upmu_interrupt_chrdet_int_en] val=%d.\r\n", val);
...
kal_uint32 upmu_get_rgs_chrdet(void)
{
    ...
    pr_debug("[upmu_get_rgs_chrdet] CHRDET status = %d\n", val);
...
{
    pr_debug( "[register_low_battery_notify] start\n");

    lbcb_tb[prio_val].lbcb = low_battery_callback;

    pr_debug( "[register_low_battery_notify] prio_val=%d\n",prio_val);
}
...
void exec_low_battery_callback(LOW_BATTERY_LEVEL low_battery_level) //0:no limit
{
    ...
    {
        pr_debug( "[exec_low_battery_callback] g_low_battery_stop=%d\n", g_low_battery_stop);
    }
    else
    {
...
void low_battery_protect_init(void)
{
    ...
    pr_debug( "[low_battery_protect_init] %d mV, %d mV, %d mV\n",
        POWER_INT0_VOLT, POWER_INT1_VOLT, POWER_INT2_VOLT);
    pr_debug( "[low_battery_protect_init] Done\n");
...
{
    pr_debug( "[register_battery_oc_notify] start\n");

    occb_tb[prio_val].occb = battery_oc_callback;

    pr_debug( "[register_battery_oc_notify] prio_val=%d\n",prio_val);
}
...
void exec_battery_oc_callback(BATTERY_OC_LEVEL battery_oc_level) //0:no limit
{
    ...
    {
        pr_debug( "[exec_battery_oc_callback] g_battery_oc_stop=%d\n", g_battery_oc_stop);
    }
    else
    {
        for(i=0 ; i<OCCB_NUM ; i++)
        {
            if(occb_tb[i].occb != NULL)
            {
                ...
                pr_debug( "[exec_battery_oc_callback] prio_val=%d,battery_oc_level=%d\n",i,battery_oc_level);
            }
        }
    }
}
...
void battery_oc_protect_init(void)
{
    ...
    pr_debug( "[battery_oc_protect_init] %d mA, %d mA\n",
        POWER_BAT_OC_CURRENT_H, POWER_BAT_OC_CURRENT_L);
        pr_debug( "[battery_oc_protect_init] Done\n");
}
...
void battery_oc_protect_reinit(void)
{
    ...
    pr_debug( "[battery_oc_protect_reinit] %d mA, %d mA\n",
        POWER_BAT_OC_CURRENT_H_RE, POWER_BAT_OC_CURRENT_L_RE);
    pr_debug( "[battery_oc_protect_reinit] Done\n");
#else
    pr_debug( "[battery_oc_protect_reinit] no define BATTERY_OC_PROTECT\n");
#endif
}
...
{
    pr_debug( "[register_battery_percent_notify] start\n");

    bpcb_tb[prio_val].bpcb = battery_percent_callback;

    pr_debug( "[register_battery_percent_notify] prio_val=%d\n",prio_val);

    if( (g_battery_percent_stop==0) && (g_battery_percent_level==1) )
    {
        pr_debug( "[register_battery_percent_notify] level l happen\n");
...
void exec_battery_percent_callback(BATTERY_PERCENT_LEVEL battery_percent_level) //0:no limit
{
    ...
    {
        pr_debug( "[exec_battery_percent_callback] g_battery_percent_stop=%d\n", g_battery_percent_stop);
    }
    else
    {
        for(i=0 ; i<BPCB_NUM ; i++)
        {
            if(bpcb_tb[i].bpcb != NULL)
            {
                ...
                pr_debug( "[exec_battery_percent_callback] prio_val=%d,battery_percent_level=%d\n",i,battery_percent_level);
            }
        }
    }
}
...
int bat_percent_notify_handler(void *unused)
{
    ...
    do
    {
        ...
        pr_debug( "bat_per_level=%d,bat_per_val=%d\n",g_battery_percent_level,bat_per_val);
...
enum hrtimer_restart bat_percent_notify_task(struct hrtimer *timer)
{
    ...
    pr_debug( "bat_percent_notify_task is called\n");
...
{
    pr_debug("no kpd_pwrkey_pmic_handler at FPGA\n");
}
#else
...
void pwrkey_int_handler(void)
{
    ...
    pr_debug( "[pwrkey_int_handler]....\n");

    if(mt6325_upmu_get_pwrkey_deb()==1)
    {
        pr_debug( "[pwrkey_int_handler] Release pwrkey\n");
        ...
        if(g_boot_mode == KERNEL_POWER_OFF_CHARGING_BOOT && timer_pre != 0)
        {
                ...
                pr_debug( "[pmic_thread_kthread] timer_pos = %ld, timer_pre = %ld, timer_pos-timer_pre = %ld, long_pwrkey_press = %d\r\n",timer_pos, timer_pre, timer_pos-timer_pre, long_pwrkey_press);
                if(long_pwrkey_press)   //500ms
                {
                    pr_debug( "[pmic_thread_kthread] Power Key Pressed during kernel power off charging, reboot OS\r\n");
                    arch_reset(0, NULL);
                }
        }
        #endif
    ...
    {
        pr_debug( "[pwrkey_int_handler] Press pwrkey\n");
...
{
    pr_debug("no kpd_pmic_rstkey_handler at FPGA\n");
}
#else
...
void homekey_int_handler(void)
{
    ...
    pr_debug( "[homekey_int_handler]....\n");

    if(mt6325_upmu_get_homekey_deb()==1)
    {
        pr_debug( "[homekey_int_handler] Release homekey\n");
        kpd_pmic_rstkey_handler(0x0);
        //mt6325_upmu_set_rg_homekey_int_sel(0);
    }
    else
    {
        pr_debug( "[homekey_int_handler] Press homekey\n");
...
{
    kal_uint32 ret=0;
    pr_debug( "[homekey_r_int_handler]....\n");
    //sub-task
...
void bat_l_int_handler(void)
{
    ...
    if(g_low_battery_level==1)
        ...
        pr_debug( "[bat_l_int_handler]err,g_low_battery_level=%d\n", g_low_battery_level);
...
{
    kal_uint32 ret=0;
    pr_debug( "[vow_int_handler]....\n");

    #if defined (MTK_VOW_SUPPORT) && defined (MTK_VOW_TEST)
    pr_debug("vow_int_handler, vow_irq_handler\n");
    vow_irq_handler();
    #endif
    //sub-task
...
{
    kal_uint32 ret=0;
    pr_debug( "[accdet_int_handler]....\n");

	#if defined(CONFIG_MTK_ACCDET)
	ret = accdet_irq_handler();
	#endif
	if(0 == ret){
		pr_debug( "[accdet_int_handler] don't finished\n");
	}
...
{
    kal_uint32 ret=0;
    pr_debug( "[accdet_eint_int_handler]....\n");
    //sub-task
...
{
    kal_uint32 ret=0;
    pr_debug( "[accdet_negv_int_handler]....\n");
    //sub-task
...
{
    kal_uint32 ret=0;
    pr_debug( "[vdvfs11_oc_int_handler]....\n");
    //sub-task
...
{
    kal_uint32 ret=0;
    pr_debug( "[vdvfs12_oc_int_handler]....\n");
    //sub-task
...
{
    kal_uint32 ret=0;
    pr_debug( "[vrf18_0_oc_int_handler]....\n");
    //sub-task
...
{
    kal_uint32 ret=0;
    pr_debug( "[vismps0_h_int_handler]....\n");
    //sub-task
...
{
    kal_uint32 ret=0;
    pr_debug( "[vismps0_l_int_handler]....\n");
    //sub-task
...
{
    kal_uint32 ret=0;
    pr_debug( "[auxadc_imp_int_handler]....\n");
    //sub-task
...
{
    kal_uint32 ret=0;
    pr_debug( "[bvalid_det_int_handler]....\n");
    //sub-task
...
{
    kal_uint32 ret=0;
    pr_debug( "[vbaton_undet_int_handler]....\n");
    //sub-task
...
{
    kal_uint32 ret=0;
    pr_debug( "[pchr_cm_vdec_int_handler]....\n");
    //sub-task
...
void chrdet_int_handler(void)
{
    ...
    if (!upmu_get_rgs_chrdet())
    {
        ...
        {
            pr_debug( "[chrdet_int_handler] Unplug Charger/USB In Kernel Power Off Charging Mode!  Shutdown OS!\r\n");
            mt_power_off();
        }
    }
    #endif
    ...
    //pr_debug( "Before charging task, mt6325_upmu_set_rg_usbdl_rst(1)\n");
...
{
    kal_uint32 ret=0;
    pr_debug( "[pchr_cm_vinc_int_handler]....\n");
    //sub-task
...
static int pmic_thread_kthread_mt6325(void *x)
{
    ...
    while (1) {
        ...
        pr_debug( "[PMIC_INT] pwrap_eint_status=0x%x\n", pwrap_eint_status);

        mt6325_int_handler();

        pmic_wrap_eint_clr(0x0);
        pr_debug( "[PMIC_INT] pmic_wrap_eint_clr(0x0);\n");
        ...
        pr_debug( "[PMIC_INT] after ,mt6325_int_status_val_0=0x%x\n", mt6325_int_status_val_0);
        pr_debug( "[PMIC_INT] after ,mt6325_int_status_val_1=0x%x\n", mt6325_int_status_val_1);
        pr_debug( "[PMIC_INT] after ,mt6325_int_status_val_2=0x%x\n", mt6325_int_status_val_2);
...
{
    pr_debug( "[mt_pmic_eint_irq_mt6325] receive interrupt\n");
    wake_up_pmic_mt6325();
    return ;
}
...
void PMIC_EINT_SETTING(void)
{
	...
	pr_debug( "[CUST_EINT] request_irq=%d\n", ret);

	pr_debug( "[CUST_EINT] mt6325_irq=%d\n", g_mt6325_irq);
	pr_debug( "[CUST_EINT] CUST_EINT_PMIC_DEBOUNCE_CN=%d\n", g_cust_eint_mt_pmic_debounce_cn);
	pr_debug( "[CUST_EINT] CUST_EINT_PMIC_TYPE=%d\n", g_cust_eint_mt_pmic_type);
	pr_debug( "[CUST_EINT] CUST_EINT_PMIC_DEBOUNCE_EN=%d\n", g_cust_eint_mt_pmic_debounce_en);
#else
    ...
    pr_debug( "[CUST_EINT] CUST_EINT_MT_PMIC_MT6325_NUM=%d\n", g_eint_pmit_mt6325_num);
    pr_debug( "[CUST_EINT] CUST_EINT_PMIC_DEBOUNCE_CN=%d\n", g_cust_eint_mt_pmic_debounce_cn);
    pr_debug( "[CUST_EINT] CUST_EINT_PMIC_TYPE=%d\n", g_cust_eint_mt_pmic_type);
    pr_debug( "[CUST_EINT] CUST_EINT_PMIC_DEBOUNCE_EN=%d\n", g_cust_eint_mt_pmic_debounce_en);
#endif
...
U32 pmic_read_interface (U32 RegNum, U32 *val, U32 MASK, U32 SHIFT)
{
    ...
    if(return_value!=0)
    {
        pr_debug( "[pmic_read_interface] Reg[%x]= pmic_wrap read data fail\n", RegNum);
    ...
    *val = (pmic_reg >> SHIFT);
    ...
    mutex_unlock(&pmic_access_mutex);
#else
    //pr_debug( "[pmic_read_interface] Can not access HW PMIC\n");
#endif
...
U32 pmic_config_interface (U32 RegNum, U32 val, U32 MASK, U32 SHIFT)
{
    ...
    if(return_value!=0)
    {
        pr_debug( "[pmic_config_interface] Reg[%x]= pmic_wrap read data fail\n", RegNum);
        mutex_unlock(&pmic_access_mutex);
        return return_value;
    }
    ...
    if(return_value!=0)
    {
        pr_debug( "[pmic_config_interface] Reg[%x]= pmic_wrap write data fail\n", RegNum);
        mutex_unlock(&pmic_access_mutex);
        return return_value;
    }
    //pr_debug( "[pmic_config_interface] Reg[%x]=0x%x\n", RegNum, pmic_reg);
    ...
    mutex_unlock(&pmic_access_mutex);
#else
    //pr_debug( "[pmic_config_interface] Can not access HW PMIC\n");
#endif
...
U32 pmic_read_interface_nolock (U32 RegNum, U32 *val, U32 MASK, U32 SHIFT)
{
    ...
    if(return_value!=0)
    {
        pr_debug( "[pmic_read_interface_nolock] Reg[%x]= pmic_wrap read data fail\n", RegNum);
        return return_value;
    }
...
#else
    pr_debug( "[pmic_read_interface_nolock] Can not access HW PMIC\n");
#endif
...
U32 pmic_config_interface_nolock (U32 RegNum, U32 val, U32 MASK, U32 SHIFT)
{
    ...
    if(return_value!=0)
    {
        pr_debug( "[pmic_config_interface_nolock] Reg[%x]= pmic_wrap read data fail\n", RegNum);
        return return_value;
    }
...
#else
    pr_debug( "[pmic_config_interface_nolock] Can not access HW PMIC\n");
#endif
...
static ssize_t store_pmic_access(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
    ...
    if(buf != NULL && size != 0)
    {
        pr_debug( "[store_pmic_access] buf is %s and size is %zu \n",buf,size);
        ...
        else
        {
            ...
            pr_debug( "[store_pmic_access] Please use \"cat pmic_access\" to get value\r\n");
        }
    }
    return size;
}
...
{
    pr_debug( "[dct_pmic_VTCXO0_enable] %d\n", dctEnable);
...
{
    pr_debug( "[dct_pmic_VTCXO1_enable] %d\n", dctEnable);
...
{
    pr_debug( "[dct_pmic_VAUD28_enable] %d\n", dctEnable);
...
{
    pr_debug( "[dct_pmic_VAUXA28_enable] %d\n", dctEnable);
...
{
    pr_debug( "[dct_pmic_VBIF28_enable] %d\n", dctEnable);
...
{
    pr_debug( "[dct_pmic_VCAMA_enable] %d\n", dctEnable);
...
{
    pr_debug( "[dct_pmic_VCN28_enable] %d\n", dctEnable);
...
{
    pr_debug( "[dct_pmic_VCN33_enable] %d\n", dctEnable);
...
{
    pr_debug( "[dct_pmic_VRF18_1_enable] %d\n", dctEnable);
...
{
    pr_debug( "[dct_pmic_VUSB33_enable] %d\n", dctEnable);
...
{
    pr_debug( "[dct_pmic_VMCH_enable] %d\n", dctEnable);
...
{
    pr_debug( "[dct_pmic_VMC_enable] %d\n", dctEnable);
...
{
    pr_debug( "[dct_pmic_VEMC33_enable] %d\n", dctEnable);
...
{
    pr_debug( "[dct_pmic_VIO28_enable] %d\n", dctEnable);
...
{
    pr_debug( "[dct_pmic_VCAM_AF_enable] %d\n", dctEnable);
...
{
    pr_debug( "[dct_pmic_VGP1_enable] %d\n", dctEnable);
...
{
    pr_debug( "[dct_pmic_VEFUSE_enable] %d\n", dctEnable);
...
{
    pr_debug( "[dct_pmic_VSIM1_enable] %d\n", dctEnable);
...
{
    pr_debug( "[dct_pmic_VSIM2_enable] %d\n", dctEnable);
...
{
    pr_debug( "[dct_pmic_VMIPI_enable] %d\n", dctEnable);
...
{
    pr_debug( "[dct_pmic_VCN18_enable] %d\n", dctEnable);
...
{
    pr_debug( "[dct_pmic_VGP2_enable] %d\n", dctEnable);
...
{
    pr_debug( "[dct_pmic_VCAMD_enable] %d\n", dctEnable);
...
{
    pr_debug( "[dct_pmic_VCAM_IO_enable] %d\n", dctEnable);
...
{
    pr_debug( "[dct_pmic_VSRAM_DVFS1_enable] %d\n", dctEnable);
...
{
    pr_debug( "[dct_pmic_VGP3_enable] %d\n", dctEnable);
...
{
    pr_debug( "[dct_pmic_VBIASN_enable] %d\n", dctEnable);
...
{
    pr_debug( "[dct_pmic_VRTC_enable] %d\n", dctEnable);
...
{
    pr_debug( "****[dct_pmic_VTCXO0_sel] no HW support\n");
}

void dct_pmic_VTCXO1_sel(kal_uint32 volt)
{
    pr_debug( "****[dct_pmic_VTCXO1_sel] no HW support\n");
}

void dct_pmic_VAUD28_sel(kal_uint32 volt)
{
    pr_debug( "****[dct_pmic_VAUD28_sel] no HW support\n");
}

void dct_pmic_VAUXA28_sel(kal_uint32 volt)
{
    pr_debug( "****[dct_pmic_VAUXA28_sel] no HW support\n");
}

void dct_pmic_VBIF28_sel(kal_uint32 volt)
{
    pr_debug( "****[dct_pmic_VBIF28_sel] no HW support\n");
}

void dct_pmic_VCAMA_sel(kal_uint32 volt)
{
    pr_debug( "****[dct_pmic_VCAMA_sel] value=%d \n", volt);
    ...
    if(volt == VOL_DEFAULT)  {mt6325_upmu_set_rg_vcama_vosel(3);}
        ...
        pr_debug( "Error Setting %d. DO nothing.\r\n", volt);
    }
}

void dct_pmic_VCN28_sel(kal_uint32 volt)
{
    pr_debug( "****[dct_pmic_VCN28_sel] no HW support\n");
}

void dct_pmic_VCN33_sel(kal_uint32 volt)
{
    pr_debug( "****[dct_pmic_VCN33_sel] value=%d \n", volt);
...
{
    pr_debug( "****[dct_pmic_VRF18_1_sel] value=%d \n", volt);
...
{
    pr_debug( "****[dct_pmic_VUSB33_sel] no HW support\n");
}

void dct_pmic_VMCH_sel(kal_uint32 volt)
{
    pr_debug( "****[dct_pmic_VMCH_sel] value=%d \n", volt);
...
{
    pr_debug( "****[dct_pmic_VMC_sel] value=%d \n", volt);
    ...
    pr_debug( "[dct_pmic_VMC_sel] [0x%x]=0x%x,[0x%x]=0x%x.\n",
...
{
    pr_debug( "****[dct_pmic_VEMC33_sel] value=%d \n", volt);
...
{
    pr_debug( "****[dct_pmic_VIO28_sel] no HW support\n");
}

void dct_pmic_VCAM_AF_sel(kal_uint32 volt)
{
    pr_debug( "****[dct_pmic_VCAM_AF_sel] value=%d \n", volt);
...
{
    pr_debug( "****[dct_pmic_VGP1_sel] value=%d \n", volt);
...
{
    pr_debug( "****[dct_pmic_VEFUSE_sel] value=%d \n", volt);
...
{
    pr_debug( "****[dct_pmic_VSIM1_sel] value=%d \n", volt);
...
{
    pr_debug( "****[dct_pmic_VSIM2_sel] value=%d \n", volt);
...
{
    pr_debug( "****[dct_pmic_VMIPI_sel] value=%d \n", volt);
...
{
    pr_debug( "****[dct_pmic_VCN18_sel] value=%d \n", volt);
...
{
    pr_debug( "****[dct_pmic_VGP2_sel] value=%d \n", volt);
...
{
    pr_debug( "****[dct_pmic_VCAMD_sel] value=%d \n", volt);
...
{
    pr_debug( "****[dct_pmic_VCAM_IO_sel] value=%d \n", volt);
...
{
    pr_debug( "****[dct_pmic_VSRAM_DVFS1_sel] not support\n");
}

void dct_pmic_VGP3_sel(kal_uint32 volt)
{
    pr_debug( "****[dct_pmic_VGP3_sel] value=%d \n", volt);
...
void dct_pmic_VBIASN_sel(kal_uint32 volt)
{
    ...
    pr_debug( "****[dct_pmic_VBIASN_sel] value=%d, val=%d \n", volt, val);
}

void dct_pmic_VRTC_sel(kal_uint32 volt)
{
    pr_debug( "****[dct_pmic_VRTC_sel] no HW support\n");
}
#endif
...
void pmic_ldo_enable(MT_POWER powerId, kal_bool powerEnable)
{
    ...
    {
        pr_debug( "[pmic_ldo_enable] UnKnown powerId (%d)\n", powerId);
    }
...
void pmic_ldo_vol_sel(MT_POWER powerId, MT_POWER_VOLTAGE powerVolt)
{
    ...
    {
        pr_debug( "[pmic_ldo_ldo_vol_sel] UnKnown powerId (%d)\n", powerId);
    }
...
void dump_ldo_status_read_debug(void)
{
    ...
    pr_debug("VDVFS11     =%d, ",  (((val_0)&(0x0001))>>0) );
    pr_debug("VDVFS12     =%d, ",  (((val_0)&(0x0002))>>1) );
    pr_debug("VDRAM       =%d, ",  (((val_0)&(0x0004))>>2) );
    pr_debug("VRF18_0     =%d\n",  (((val_0)&(0x0008))>>3) );

    pr_debug("VGPU        =%d, ",  (((val_0)&(0x0010))>>4) );
    pr_debug("VCORE1      =%d, ",  (((val_0)&(0x0020))>>5) );
    pr_debug("VCORE2      =%d, ",  (((val_0)&(0x0040))>>6) );
    pr_debug("VIO18       =%d\n",  (((val_0)&(0x0080))>>7) );

    pr_debug("VPA         =%d, ",  (((val_0)&(0x0100))>>8) );
    pr_debug("VRTC        =%d, ",  (((val_0)&(0x0200))>>9) );
    pr_debug("VTCXO0      =%d, ",  (((val_0)&(0x0400))>>10) );
    pr_debug("VTCXO1      =%d\n",  (((val_0)&(0x0800))>>11) );

    pr_debug("VAUD28      =%d, ",  (((val_0)&(0x1000))>>12) );
    pr_debug("VAUXA28     =%d, ",  (((val_0)&(0x2000))>>13) );
    pr_debug("VCAMA       =%d, ",  (((val_0)&(0x4000))>>14) );
    pr_debug("VIO28       =%d\n",  (((val_0)&(0x8000))>>15) );

    pr_debug("VCAM_AF     =%d, ",  (((val_1)&(0x0001))>>0) );
    pr_debug("VMC         =%d, ",  (((val_1)&(0x0002))>>1) );
    pr_debug("VMCH        =%d, ",  (((val_1)&(0x0004))>>2) );
    pr_debug("VEMC33      =%d\n",  (((val_1)&(0x0008))>>3) );

    pr_debug("VGP1        =%d, ",  (((val_1)&(0x0010))>>4) );
    pr_debug("VEFUSE      =%d, ",  (((val_1)&(0x0020))>>5) );
    pr_debug("VSIM1       =%d, ",  (((val_1)&(0x0040))>>6) );
    pr_debug("VSIM2       =%d\n",  (((val_1)&(0x0080))>>7) );

    pr_debug("VCN28       =%d, ",  (((val_1)&(0x0100))>>8) );
    pr_debug("VMIPI       =%d, ",  (((val_1)&(0x0200))>>9) );
    pr_debug("VIBR        =%d, ",  (((val_1)&(0x0400))>>10) );
    pr_debug("VCAMD       =%d\n",  (((val_1)&(0x0800))>>11) );

    pr_debug("VUSB33      =%d, ",  (((val_1)&(0x1000))>>12) );
    pr_debug("VCAM_IO     =%d, ",  (((val_1)&(0x2000))>>13) );
    pr_debug("VSRAM_DVFS1 =%d, ",  (((val_1)&(0x4000))>>14) );
    pr_debug("VGP2        =%d\n",  (((val_1)&(0x8000))>>15) );

    pr_debug("VGP3        =%d, ",  (((val_2)&(0x0001))>>0) );
    pr_debug("VBIASN      =%d, ",  (((val_2)&(0x0002))>>1) );
    pr_debug("VCN33       =%d, ",  (((val_2)&(0x0004))>>2) );
    pr_debug("VCN18       =%d\n",  (((val_2)&(0x0008))>>3) );

    pr_debug("VRF18_1     =%d, ",  (((val_2)&(0x0010))>>4) );
#endif
}
...
void pmic_debug_init(void)
{
    ...
    if (!mt_pmic_dir) {
        pr_debug( "fail to mkdir /proc/mt_pmic\n" );
        return;
    }

    #if 1
    proc_create("dump_ldo_status", S_IRUGO | S_IWUSR, mt_pmic_dir, &pmic_debug_proc_fops);
    pr_debug( "proc_create pmic_debug_proc_fops\n" );
    #else
...
{
    pr_debug( "[show_low_battery_protect_ut] g_low_battery_level=%d\n", g_low_battery_level);
    return sprintf(buf, "%u\n", g_low_battery_level);
}
static ssize_t store_low_battery_protect_ut(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
    ...
    pr_debug( "[store_low_battery_protect_ut] \n");

    if(buf != NULL && size != 0)
    {
        pr_debug( "[store_low_battery_protect_ut] buf is %s and size is %zu \n",buf,size);
        val = simple_strtoul(buf,&pvalue,16);
        if(val<=2)
        {
            pr_debug( "[store_low_battery_protect_ut] your input is %d\n", val);
            exec_low_battery_callback(val);
        }
        else
        {
            pr_debug( "[store_low_battery_protect_ut] wrong number (%d)\n", val);
        }
    }
    return size;
}
...
{
    pr_debug( "[show_low_battery_protect_stop] g_low_battery_stop=%d\n", g_low_battery_stop);
    return sprintf(buf, "%u\n", g_low_battery_stop);
}
static ssize_t store_low_battery_protect_stop(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
    ...
    pr_debug( "[store_low_battery_protect_stop] \n");

    if(buf != NULL && size != 0)
    {
        pr_debug( "[store_low_battery_protect_stop] buf is %s and size is %zu \n",buf,size);
        val = simple_strtoul(buf,&pvalue,16);
        if( (val!=0) && (val!=1) )
            val=0;
        g_low_battery_stop = val;
        pr_debug( "[store_low_battery_protect_stop] g_low_battery_stop=%d\n", g_low_battery_stop);
    }
    return size;
}
...
{
    pr_debug( "[show_low_battery_protect_level] g_low_battery_level=%d\n", g_low_battery_level);
    return sprintf(buf, "%u\n", g_low_battery_level);
}
static ssize_t store_low_battery_protect_level(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
    pr_debug( "[store_low_battery_protect_level] g_low_battery_level=%d\n", g_low_battery_level);
...
{
    pr_debug( "[show_battery_oc_protect_ut] g_battery_oc_level=%d\n", g_battery_oc_level);
    return sprintf(buf, "%u\n", g_battery_oc_level);
}
static ssize_t store_battery_oc_protect_ut(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
    ...
    pr_debug( "[store_battery_oc_protect_ut] \n");

    if(buf != NULL && size != 0)
    {
        pr_debug( "[store_battery_oc_protect_ut] buf is %s and size is %zu \n",buf,size);
        val = simple_strtoul(buf,&pvalue,16);
        if(val<=1)
        {
            pr_debug( "[store_battery_oc_protect_ut] your input is %d\n", val);
            exec_battery_oc_callback(val);
        }
        else
        {
            pr_debug( "[store_battery_oc_protect_ut] wrong number (%d)\n", val);
        }
    }
    return size;
}
...
{
    pr_debug( "[show_battery_oc_protect_stop] g_battery_oc_stop=%d\n", g_battery_oc_stop);
    return sprintf(buf, "%u\n", g_battery_oc_stop);
}
static ssize_t store_battery_oc_protect_stop(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
    ...
    pr_debug( "[store_battery_oc_protect_stop] \n");

    if(buf != NULL && size != 0)
    {
        pr_debug( "[store_battery_oc_protect_stop] buf is %s and size is %zu \n",buf,size);
        val = simple_strtoul(buf,&pvalue,16);
        if( (val!=0) && (val!=1) )
            val=0;
        g_battery_oc_stop = val;
        pr_debug( "[store_battery_oc_protect_stop] g_battery_oc_stop=%d\n", g_battery_oc_stop);
    }
    return size;
}
...
{
    pr_debug( "[show_battery_oc_protect_level] g_battery_oc_level=%d\n", g_battery_oc_level);
    return sprintf(buf, "%u\n", g_battery_oc_level);
}
static ssize_t store_battery_oc_protect_level(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
    pr_debug( "[store_battery_oc_protect_level] g_battery_oc_level=%d\n", g_battery_oc_level);
...
{
    pr_debug( "[show_battery_percent_protect_ut] g_battery_percent_level=%d\n", g_battery_percent_level);
    return sprintf(buf, "%u\n", g_battery_percent_level);
}
static ssize_t store_battery_percent_protect_ut(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
    ...
    if(buf != NULL && size != 0)
    {
        pr_debug( "[store_battery_percent_protect_ut] buf is %s and size is %zu \n",buf,size);
        val = simple_strtoul(buf,&pvalue,16);
        if(val<=1)
        {
            pr_debug( "[store_battery_percent_protect_ut] your input is %d\n", val);
            exec_battery_percent_callback(val);
        }
        else
        {
            pr_debug( "[store_battery_percent_protect_ut] wrong number (%d)\n", val);
        }
    }
    return size;
}
...
{
    pr_debug( "[show_battery_percent_protect_stop] g_battery_percent_stop=%d\n", g_battery_percent_stop);
    return sprintf(buf, "%u\n", g_battery_percent_stop);
}
static ssize_t store_battery_percent_protect_stop(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
    ...
    if(buf != NULL && size != 0)
    {
        pr_debug( "[store_battery_percent_protect_stop] buf is %s and size is %zu \n",buf,size);
        val = simple_strtoul(buf,&pvalue,16);
        if( (val!=0) && (val!=1) )
            val=0;
...
{
    pr_debug( "[show_battery_percent_protect_level] g_battery_percent_level=%d\n", g_battery_percent_level);
    return sprintf(buf, "%u\n", g_battery_percent_level);
}
...
{
    pr_debug( "[store_battery_percent_protect_level] g_battery_percent_level=%d\n", g_battery_percent_level);
...
static ssize_t store_pmic_dvt(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
    ...
    pr_debug("[store_pmic_dvt] \n");

    if(buf != NULL && size != 0)
    {
        pr_debug("[store_pmic_dvt] buf is %s and size is %zu\n",buf,size);
        test_item = simple_strtoul(buf,&pvalue,10);
        pr_debug("[store_pmic_dvt] test_item=%d\n", test_item);

        #ifdef MTK_PMIC_DVT_SUPPORT
        pmic_dvt_entry(test_item);
        #else
        pr_debug("[store_pmic_dvt] no define MTK_PMIC_DVT_SUPPORT\n");
        #endif
    }
    return size;
}
...
int is_ext_swchr_exist(void)
{
        ...
        if( (is_bq24261_exist()==1) )
            ...
            return 0;
    #else
        pr_debug( "[is_ext_swchr_exist] no define any HW\n");
        return 0;
    #endif
}
...
void pmic_full_reset(void)
{
    ...
    ret_val=pmic_config_interface(MT6325_TOP_RST_MISC_SET, 0x0001, 0xFFFF, 0); //[0]=1, RG_WDTRSTB_EN
    pr_debug("[pmic_full_reset] Reg[0x%x]=0x%x\n", MT6325_TOP_RST_MISC, upmu_get_reg_value(MT6325_TOP_RST_MISC));
#endif
}
...
void PMIC_INIT_SETTING_V1(void)
{
    ...
    {
        pr_debug( "[Kernel_PMIC_INIT_SETTING_V1] 6325 PMIC Chip = 0x%x\n",mt6325_chip_version);
        pr_debug( "[Kernel_PMIC_INIT_SETTING_V1] 2014-08-20\n");
    ...
    {
        pr_debug( "[Kernel_PMIC_INIT_SETTING_V1] 6325 PMIC Chip = 0x%x\n",mt6325_chip_version);
        pr_debug( "[Kernel_PMIC_INIT_SETTING_V1] 2014-08-15\n");
    ...
    {
        pr_debug( "[Kernel_PMIC_INIT_SETTING_V1] Unknown PMIC Chip (0x%x)\n",mt6325_chip_version);
    }

    #if 1
        pr_debug( "[mt6325] [0x%x]=0x%x\n", 0x1E, upmu_get_reg_value(0x1E));
    ...
    switch(cmd)
    {
        //#if defined(FTM_EXT_BUCK_CHECK)
            case Get_IS_EXT_BUCK_EXIST:
                ...
                pr_debug( "[pmic_ftm_ioctl] Get_IS_EXT_BUCK_EXIST:%d\n", adc_out_data[0]);
            break;
        //#endif
        ...
        //#if defined(FTM_EXT_VBAT_BOOST_CHECK)
            case Get_IS_EXT_VBAT_BOOST_EXIST:
                ...
                ret = copy_to_user(user_data_addr, adc_out_data, 8);
                pr_debug( "[pmic_ftm_ioctl] Get_IS_EXT_VBAT_BOOST_EXIST:%d\n", adc_out_data[0]);
            break;
        //#endif
        ...
        //#if defined(FEATURE_FTM_SWCHR_HW_DETECT)
            case Get_IS_EXT_SWCHR_EXIST:
                ...
                ret = copy_to_user(user_data_addr, adc_out_data, 8);
                pr_debug( "[pmic_ftm_ioctl] Get_IS_EXT_SWCHR_EXIST:%d\n", adc_out_data[0]);
            break;
        //#endif

        default:
            pr_debug( "[pmic_ftm_ioctl] Error ID\n");
            break;
    }
...
void pmic_ftm_init(void)
{
    ...
    if (ret)
        pr_debug( "[pmic_ftm_init] Error: Can't Get Major number for pmic_ftm\n");
    ...
    if(ret)
        pr_debug( "[pmic_ftm_init] Error: cdev_add\n");
...
static int pmic_mt_probe(struct platform_device *dev)
{
    ...
    //get PMIC CID
    pr_debug( "MT6325 PMIC CID=0x%x.\n", get_mt6325_pmic_chip_version());

    //pmic initial setting
    PMIC_INIT_SETTING_V1();
    pr_debug( "[PMIC_INIT_SETTING_V1] Done\n");
    PMIC_CUSTOM_SETTING_V1();
    pr_debug( "[PMIC_CUSTOM_SETTING_V1] Done\n");
...
#if defined(CONFIG_MTK_FPGA)
    pr_debug( "[PMIC_EINT_SETTING] disable when CONFIG_MTK_FPGA\n");
#else
    //PMIC Interrupt Service
    ...
    if (IS_ERR(pmic_6325_thread_handle))
    {
        ...
        pr_debug( "[pmic_thread_kthread_mt6325] creation fails\n");
    }
    else
    {
        wake_up_process(pmic_6325_thread_handle);
        pr_debug( "[pmic_thread_kthread_mt6325] kthread_create Done\n");
    }

    PMIC_EINT_SETTING();
    pr_debug( "[PMIC_EINT_SETTING] Done\n");
#endif
    mt6325_upmu_set_rg_pwrkey_int_sel(1);
    mt6325_upmu_set_rg_homekey_int_sel(1);
    pr_debug( "[PMIC KEY] Reg[0x%x]=0x%x\n", MT6325_INT_MISC_CON, upmu_get_reg_value(MT6325_INT_MISC_CON));
    ...
    pmic_debug_init();
    pr_debug( "[PMIC] pmic_debug_init : done.\n" );
    ...
    pr_debug( "[PMIC] device_create_file for EM : done.\n" );
#endif
...
{
    pr_debug( "******** MT pmic driver remove!! ********\n" );
...
{
	U32 adc_busy;
    pr_debug( "******** MT pmic driver resume!! ********\n" );
...
{
    pr_debug( "******** MT pmic driver early suspend!! ********\n" );
    mt6325_upmu_set_rg_auxadc_32k_ck_pdn(0x1);
...
{
    pr_debug( "******** MT pmic driver early resume!! ********\n" );
    mt6325_upmu_set_rg_auxadc_32k_ck_pdn(0x0);
...
static int __init pmic_mt_init(void)
{
    ...
    if (ret) {
        pr_debug( "****[pmic_mt_init] Unable to device register(%d)\n", ret);
        return ret;
    }
    ret = platform_driver_register(&pmic_mt_driver);
    if (ret) {
        pr_debug( "****[pmic_mt_init] Unable to register driver (%d)\n", ret);
        return ret;
    }
