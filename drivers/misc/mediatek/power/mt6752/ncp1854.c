...
kal_uint32 ncp1854_read_interface (kal_uint8 RegNum, kal_uint8 *val, kal_uint8 MASK, kal_uint8 SHIFT)
{
    ...
    pr_debug("[ncp1854_read_interface] Reg[%x]=0x%x\n", RegNum, ncp1854_reg);

    ncp1854_reg &= (MASK << SHIFT);
    *val = (ncp1854_reg >> SHIFT);
    pr_debug("[ncp1854_read_interface] Val=0x%x\n", *val);
...
kal_uint32 ncp1854_config_interface (kal_uint8 RegNum, kal_uint8 val, kal_uint8 MASK, kal_uint8 SHIFT)
{
    ...
    ret = ncp1854_read_byte(RegNum, &ncp1854_reg);
    //pr_debug("[ncp1854_config_interface] Reg[%x]=0x%x\n", RegNum, ncp1854_reg);

    ncp1854_reg &= ~(MASK << SHIFT);
    ncp1854_reg |= (val << SHIFT);

    ret = ncp1854_write_byte(RegNum, ncp1854_reg);
    //pr_debug("[ncp18546_config_interface] Write Reg[%x]=0x%x\n", RegNum, ncp1854_reg);

    // Check
    //ncp1854_read_byte(RegNum, &ncp1854_reg);
    //pr_debug("[ncp1854_config_interface] Check Reg[%x]=0x%x\n", RegNum, ncp1854_reg);
...
void ncp1854_dump_register(void)
{
    ...
    for (i=0;i<ncp1854_REG_NUM;i++)
    {
        ...
        pr_debug("[ncp1854_dump_register] Reg[0x%X]=0x%X\n", i, ncp1854_reg[i]);
    }
}
...
{
    ncp1854_read_byte(i, &ncp1854_reg[i]);
    pr_debug("[ncp1854_read_register] Reg[0x%X]=0x%X\n", i, ncp1854_reg[i]);
}
...
{
    pr_debug("[show_ncp1854_access] 0x%x\n", g_reg_value_ncp1854);
...
static ssize_t store_ncp1854_access(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
    ...
    pr_debug("[store_ncp1854_access] \n");

    if(buf != NULL && size != 0)
    {
        pr_debug("[store_ncp1854_access] buf is %s and size is %zu \n",buf,size);
        reg_address = simple_strtoul(buf,&pvalue,16);

        if(size > 3)
        {
            reg_value = simple_strtoul((pvalue+1),NULL,16);
            pr_debug("[store_ncp1854_access] write ncp1854 reg 0x%x with value 0x%x !\n",reg_address,reg_value);
        ...
        else
        {
            ...
            pr_debug("[store_ncp1854_access] read ncp1854 reg 0x%x with value 0x%x !\n",reg_address,g_reg_value_ncp1854);
            pr_debug("[store_ncp1854_access] Please use \"cat ncp1854_access\" to get value\r\n");
        }
    }
    return size;
}
...
static int ncp1854_user_space_probe(struct platform_device *dev)
{
    ...
    pr_debug("******** ncp1854_user_space_probe!! ********\n" );
...
static int __init ncp1854_init(void)
{
    ...
    pr_debug("[ncp1854_init] init start\n");

    i2c_register_board_info(NCP1854_BUSNUM, &i2c_ncp1854, 1);

    if(i2c_add_driver(&ncp1854_driver)!=0)
    {
        pr_debug("[ncp1854_init] failed to register ncp1854 i2c driver.\n");
    }
    else
    {
        pr_debug("[ncp1854_init] Success to register ncp1854 i2c driver.\n");
    }

    // ncp1854 user space access interface
    ret = platform_device_register(&ncp1854_user_space_device);
    if (ret) {
        pr_debug("****[ncp1854_init] Unable to device register(%d)\n", ret);
        return ret;
    }
    ret = platform_driver_register(&ncp1854_user_space_driver);
    if (ret) {
        pr_debug("****[ncp1854_init] Unable to register driver (%d)\n", ret);
        return ret;
    }
