...
kal_uint32 mt6311_read_byte(kal_uint8 cmd, kal_uint8 *returnData)
{
    ...
    if (ret < 0)
    {
        pr_debug("[mt6311_read_byte] ret=%d\n", ret);
...
kal_uint32 mt6311_write_byte(kal_uint8 cmd, kal_uint8 writeData)
{
    ...
    if (ret < 0)
    {
        pr_debug("[mt6311_write_byte] ret=%d\n", ret);
...
{
#if 0
    pr_debug("[mt6311_read_interface] HW no mt6311\n");
    *val = 0;
    return 1;
#else
    ...
    //pr_debug("[mt6311_read_interface] Reg[%x]=0x%x\n", RegNum, mt6311_reg);

    mt6311_reg &= (MASK << SHIFT);
    *val = (mt6311_reg >> SHIFT);

    //pr_debug("[mt6311_read_interface] val=0x%x\n", *val);
...
{
#if 0
    pr_debug("[mt6311_config_interface] HW no mt6311\n");
    return 1;
#else
    kal_uint8 mt6311_reg = 0;
    kal_uint32 ret = 0;
    ...
    ret = mt6311_read_byte(RegNum, &mt6311_reg);
    //pr_debug("[mt6311_config_interface] Reg[%x]=0x%x\n", RegNum, mt6311_reg);

    mt6311_reg &= ~(MASK << SHIFT);
    mt6311_reg |= (val << SHIFT);

    ret = mt6311_write_byte(RegNum, mt6311_reg);
    //pr_debug("[mt6311_config_interface] write Reg[%x]=0x%x\n", RegNum, mt6311_reg);

    // Check
    //ret = mt6311_read_byte(RegNum, &mt6311_reg);
    //pr_debug("[mt6311_config_interface] Check Reg[%x]=0x%x\n", RegNum, mt6311_reg);
...
kal_uint32 update_mt6311_chip_id(void)
{
    ...
    pr_debug("[update_mt6311_chip_id] id_l=0x%x, id_r=0x%x, id=0x%x\n", id_l, id_r, id);
...
kal_uint32 mt6311_get_chip_id(void)
{
    ...
    pr_debug("[mt6311_get_chip_id] g_mt6311_cid=0x%x\n", g_mt6311_cid);
...
void mt6311_hw_init(void)
{
    ...
    pr_debug("[mt6311_hw_init] 20140513, CC Lee\n");
    ...
    {
        pr_debug( "[mt6311_hw_init] 6311 PMIC Chip = 0x%x\n",mt6311_get_chip_id());
        pr_debug( "[mt6311_hw_init] 2014-11-3\n");
    ...
    {
        pr_debug( "[mt6311_hw_init] Unknown PMIC Chip (0x%x)\n",mt6311_get_chip_id());
    }
    //--------------------------------------------------------
}
...
void mt6311_hw_component_detect(void)
{
    ...
    pr_debug("[mt6311_hw_component_detect] exist=%d\n", g_mt6311_hw_exist);
}
...
{
    pr_debug( "[mt_pmic_eint_irq_mt6311] receive interrupt\n");
    wake_up_pmic_mt6311();
    return ;
}
...
{
    //kal_uint32 ret=0;
    pr_debug( "[mt6311_lbat_min_int_handler]....\n");
...
{
    //kal_uint32 ret=0;
    pr_debug( "[mt6311_lbat_max_int_handler]....\n");
...
{
	thr_l_int_status=0;
    pr_debug( "[mt6311_clr_thr_l_int_status]....\n");
}

void mt6311_clr_thr_h_int_status(void)
{
	thr_h_int_status=0;
    pr_debug( "[mt6311_clr_thr_h_int_status]....\n");
}
kal_uint32 mt6311_get_thr_l_int_status(void)
{
    pr_debug( "[mt6311_get_thr_l_int_status]....\n");
...
{
    pr_debug( "[mt6311_get_thr_h_int_status]....\n");
...
void mt6311_thr_l_int_handler(void)
{
    //kal_uint32 ret=0;
    thr_l_int_status =1;
    pr_debug( "[mt6311_thr_l_int_handler]....\n");
    //return thr_l_int_status;
...
void mt6311_thr_h_int_handler(void)
{
    //kal_uint32 ret=0;
    thr_h_int_status =1;
    pr_debug( "[mt6311_thr_h_int_handler]....\n");
...
{
    //kal_uint32 ret=0;
    pr_debug( "[mt6311_buck_oc_int_handler]....\n");
...
static void mt6311_int_handler(void)
{
    ...
    pr_debug( "[MT6311_INT] mt6311_int_status_val_0=0x%x\n", mt6311_int_status_val_0);
...
static int pmic_thread_kthread_mt6311(void *x)
{
    ...
    while (1) {
        ...
        pr_debug( "[MT6311_INT] after ,mt6311_int_status_val_0=0x%x\n", mt6311_int_status_val_0);
...
void mt6311_eint_setting(void)
{
    ...
    pr_debug( "[CUST_EINT] CUST_EINT_MT_PMIC_MT6311_NUM=%d\n", g_eint_pmic_mt6311_num);
...
void mt6311_eint_init(void)
{
    //---------------------
#if defined(CONFIG_MTK_FPGA)
    pr_debug( "[MT6311_EINT] disable when CONFIG_MTK_FPGA\n");
#else
    ...
    if (IS_ERR(pmic_6311_thread_handle))
    {
        ...
        pr_debug( "[MT6311_EINT] creation fails\n");
    }
    else
    {
        wake_up_process(pmic_6311_thread_handle);
        pr_debug( "[MT6311_EINT] kthread_create Done\n");
    }

    //mt6311_eint_setting();
    pr_debug( "[MT6311_EINT] TBD\n");
#endif
...
void cpu_low_power_setting(void)
{
        ...
        pr_debug("[cpu_low_power_setting] define PMIC_VDVFS_CUST_ENABLE\n");
    #else
        ...
        pr_debug("[cpu_low_power_setting] [0x%x]=0x%x,[0x%x]=0x%x,[0x%x]=0x%x,[0x%x]=0x%x\n",
...
static int mt6311_driver_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    pr_debug("[mt6311_driver_probe] g_mt6311_hw_exist=%d, g_mt6311_driver_ready=%d\n",
    ...
    if(g_mt6311_hw_exist==0)
    {
        ...
        pr_debug("[mt6311_driver_probe] return err\n");
        return err;
    }
...
{
    pr_debug("[show_mt6311_access] 0x%x\n", g_reg_value_mt6311);
...
static ssize_t store_mt6311_access(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
    ...
    pr_debug("[store_mt6311_access] \n");

    if(buf != NULL && size != 0)
    {
        pr_debug("[store_mt6311_access] buf is %s and size is %zu \n",buf,size);
        reg_address = simple_strtoul(buf,&pvalue,16);

        if(size > 4)
        {
            reg_value = simple_strtoul((pvalue+1),NULL,16);
            pr_debug("[store_mt6311_access] write mt6311 reg 0x%x with value 0x%x !\n",reg_address,reg_value);
        ...
        else
        {
            ...
            pr_debug("[store_mt6311_access] read mt6311 reg 0x%x with value 0x%x !\n",reg_address,g_reg_value_mt6311);
            pr_debug("[store_mt6311_access] Please use \"cat mt6311_access\" to get value\r\n");
        }
    }
    return size;
}
...
{
    pr_debug("[show_mt6311_vosel_pin] g_mt6311_vosel_pin=%d\n", g_mt6311_vosel_pin);
    return sprintf(buf, "%u\n", g_mt6311_vosel_pin);
}
static ssize_t store_mt6311_vosel_pin(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
    ...
    pr_debug( "[store_mt6311_vosel_pin] \n");

    val = simple_strtoul(buf,&pvalue,16);
    g_mt6311_vosel_pin = val;

    pr_debug( "[store_mt6311_vosel_pin] g_mt6311_vosel_pin(%d)\n", g_mt6311_vosel_pin);
...
static int mt6311_user_space_probe(struct platform_device *dev)
{
    ...
    pr_debug("******** mt6311_user_space_probe!! ********\n" );
...
static int __init mt6311_init(void)
{
    ...
    pr_debug("[mt6311_init] mt6311_AUTO_DETECT_DISABLE\n");
    g_mt6311_hw_exist=0;
    g_mt6311_driver_ready=1;
    ...
    {
        pr_debug("[mt6311_init] init start. ch=%d!!\n", mt6311_BUSNUM);

        i2c_register_board_info(mt6311_BUSNUM, &i2c_mt6311, 1);

        if(i2c_add_driver(&mt6311_driver)!=0)
        {
            pr_debug("[mt6311_init] failed to register mt6311 i2c driver.\n");
        }
        else
        {
            pr_debug("[mt6311_init] Success to register mt6311 i2c driver.\n");
        }

        // mt6311 user space access interface
        ret = platform_device_register(&mt6311_user_space_device);
        if (ret) {
            pr_debug("****[mt6311_init] Unable to device register(%d)\n", ret);
            return ret;
        }
        ret = platform_driver_register(&mt6311_user_space_driver);
        if (ret) {
            pr_debug("****[mt6311_init] Unable to register driver (%d)\n", ret);
            return ret;
        }
    }
    #else
    {
        pr_debug("[mt6311_init] DCT no define EXT BUCK\n");
        g_mt6311_hw_exist=0;
        g_mt6311_driver_ready=1;
        pr_debug("[mt6311_init] g_mt6311_hw_exist=%d, g_mt6311_driver_ready=%d\n",
            g_mt6311_hw_exist, g_mt6311_driver_ready);
    }
    #endif
