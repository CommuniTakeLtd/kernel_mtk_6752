...
void pmic_auxadc_init(void)
{
	...
	pr_debug( "****[pmic_auxadc_init] DONE\n");
}
	...
	kal_uint32 ret = 0;
		...
		case AUX_ISENSE_AP:
			...
			if (PMIC_IMM_query_busy_bit() == 1) {
				pr_debug("[AUXADC] PMIC_IMM_query_busy_bit on\n");
				set_isense_flag = 0;
				return 1;
			}
		...
		default:
		        pr_debug( "[AUXADC] Invalid channel value(%d)\n", dwChannel);
...
//			dwChannel, ret_data);
	...
	case AUX_HP_AP:
		...
		break;
	case AUX_CH10_AP:
		//pr_debug( "[AUX_CH10_AP]");
		r_val_temp = 1;
		adc_result = (adc_result_temp*r_val_temp*VOLTAGE_FULL_RANGE)/ADC_PRECISE;
		break;
	case AUX_VBIF_AP:
		//pr_debug( "[AUX_VBIF_AP]");
		r_val_temp = 2;
		adc_result = (adc_result_temp*r_val_temp*VOLTAGE_FULL_RANGE)/ADC_PRECISE;
		break;
	case AUX_ADCVIN0_MD:
		...
		break;
	case AUX_ADCVIN0_GPS:
		//pr_debug( "[AUX_CH7_GPS]");
		r_val_temp = 1;
		adc_result = (adc_result_temp*r_val_temp*VOLTAGE_FULL_RANGE)/ADC_PRECISE_CH7;
		break;
	case AUX_CH0_6311:
		//pr_debug( "[AUX_CH0_6311]");
		r_val_temp = 1;
		adc_result = (adc_result_temp*r_val_temp*VOLTAGE_FULL_RANGE_6311)/ADC_PRECISE_6311;
		break;
	case AUX_CH1_6311:
		//pr_debug( "[AUX_CH1_6311]");
		r_val_temp = 1;
		adc_result = (adc_result_temp*r_val_temp*VOLTAGE_FULL_RANGE_6311)/ADC_PRECISE_6311;
		break;
	default:
	    pr_debug( "[AUXADC] Invalid channel value(%d,%d)\n", dwChannel, trimd);
	    wake_unlock(&pmicAuxadc_irq_lock);
	    return -1;
	}
	if (Enable_BATDRV_LOG == 2) {
		pr_debug( "outputdata=%d, transfer data=%d, r_val=%d.\n",
