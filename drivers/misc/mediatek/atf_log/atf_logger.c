...
size_t ipanic_atflog_buffer(void *data, unsigned char *buffer, size_t sz_buffer)
{
    ...
    //pr_debug("ipanic_atf_log: dump %d\n", count);
    rec->has_read += count;
    if (count != sz_buffer)
        last_read = true;
    return count;
...
start:
    ...
    //pr_debug("atf_log_read: finish wait\n");
    if (ret)
        return ret;
    atf_log_lock();
...
static int __init atf_log_init(void)
{
    ...
    pr_debug("atf_buf_phy_ctl: 0x%lu\n", atf_buf_phy_ctl);
    pr_debug("atf_buf_len: %u\n", atf_buf_len);
    pr_debug("atf_buf_vir_ctl: %p\n", atf_buf_vir_ctl);
    pr_debug("atf_log_vir_addr: %p\n", atf_log_vir_addr);
