...
static int enable_secondary_clusters_pwr(void)
{
	...
	{
		pr_debug("L2$ share cluster num is only 1, no needs to enable other cluster's pwr.\n");
	}
...
int disable_secondary_clusters_pwr(void)
{
	...
	if(g_l2c_share_info.share_cluster_num == 1)
	{
		pr_debug("L2$ share cluster num is only 1, no needs to disable other cluster's pwr.\n");
	}
	else if(g_l2c_share_info.share_cluster_num == 2)
	{
...
int switch_L2(enum options option)
{
	...
	if(option == BORROW_L2)
	{
		pr_debug("[%s]: borrow L2$ cost %llu ns\n", __func__, t2 - t1);
	}
	else
	{
		pr_debug("[%s]: return L2$ cost %llu ns\n", __func__, t2 - t1);
	}
