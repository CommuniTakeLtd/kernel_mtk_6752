        ...
        switch(ccmni_dev[ccmni_inx].unframe_state)
        {
...
#ifdef __PFP_KERNEL_DEBUG__                
                pr_debug("CCMNI%d: check start flag-local_raw_data[0]=0x%02x\n",ccmni_inx,local_raw_data[0]);
#endif
...
#ifdef __PFP_KERNEL_DEBUG__                
                pr_debug("CCMNI%d: check magic num-local_raw_data[0]=0x%02x\n",ccmni_inx,local_raw_data[0]);
#endif
            ...
            case PARSE_PFP_FRAME_LENGTH_FIELD_STATE:
            {
...
#ifdef __PFP_KERNEL_DEBUG__                
                pr_debug("CCMNI%d: local_raw_size=%d\n",ccmni_inx,local_raw_size);
#endif
                if (local_raw_size >= 2)
                {
...
#ifdef __PFP_KERNEL_DEBUG__
                        pr_debug("CCMNI%d: Reset decode state then continue to parse\n",ccmni_inx);
#endif
                        ccmni_dev[ccmni_inx].unframe_state = PARSE_PFP_FRAME_START_FLAG_STATE;
                        ccmni_dev[ccmni_inx].pkt_size      = 0;
                    }
                ...
                else
                {
                     ...
                     */
#ifdef __PFP_KERNEL_DEBUG__                    
                    pr_debug("CCMNI%d: not enough len bytes\n",ccmni_inx);
#endif
                    keep_parsing = 0;
                }
            ...
            case PARSE_PFP_FRAME_GET_DATA_STATE:
            {
                if (local_raw_size < ccmni_dev[ccmni_inx].pkt_size)
                {
                     ...
                     */
#ifdef __PFP_KERNEL_DEBUG__                    
                    pr_debug("CCMNI%d: not enough pkt bytes\n",ccmni_inx);
#endif
                    keep_parsing = 0;
                }
                else
                {
                    ...
                    {                       
                        ...
                        {
#ifdef __PFP_KERNEL_DEBUG__
                            pr_debug("CCMNI%d: malloc for first pkt node\n",ccmni_inx);
#endif
                            ...
                            else
                            {
                                /* Error Handle */
#ifdef __PFP_KERNEL_DEBUG__
                                pr_debug("CCMNI%d: Can't find one available complete_ippkt entry case1\n",ccmni_inx);
#endif
                        ...
                        { 
                            ...
                            else
                            {
                                /* Error Handle */
#ifdef __PFP_KERNEL_DEBUG__
                                pr_debug("CCMNI%d: Can't find one available complete_ippkt entry case2\n",ccmni_inx);
#endif                        
                                entry.try_decode_again = 1;
                                goto error_handle_return;
                            }
...
#ifdef __PFP_KERNEL_DEBUG__                        
                        pr_debug("CCMNI%d: prepare pkt node\n",ccmni_inx);
#endif
                    ...
                    else
                    {
                         ...
                         */
#ifdef __PFP_KERNEL_DEBUG__                        
                        pr_debug("CCMNI%d: not enough free space provided by cooked_data\n",ccmni_inx);
#endif
                        keep_parsing = 0;
                    }
                }
...
void traverse_pkt_list(complete_ippkt_t *node)
{
    ...
    {
#ifdef __PFP_KERNEL_DEBUG__
        pr_debug("Packet Node: data=0x%08x, size=%d\n", t_pkt_node->pkt_data, t_pkt_node->pkt_size);
#endif
