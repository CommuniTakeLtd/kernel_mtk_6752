...
static int tmd2772_init_client_call_back(struct i2c_client *client);
...
{
     int i=0 ,err = 0,j = 0;
	 u16 data[21],sum=0,data_cali=0,max=0,min=0xFFFF;
     printk("enter %s\n",__FUNCTION__);
	sum = 0;
    mdelay(50);
	 for(i = 0;i<5;i++)
...
static long tmd2772_enable_ps(struct i2c_client *client, int enable)
{
	...
	tmd2772_init_client_call_back(client);	
...
static int tmd2772_offset(struct i2c_client *client, struct PS_CALI_DATA_STRUCT *ps_data_cali)
{
	...
	{
		ps_data_cali->close =data_cali+100;
		ps_data_cali->far_away = data_cali+50;
		ps_data_cali->valid =1;
	}else{
	    ps_data_cali->close =900;
		ps_data_cali->far_away = 750;
	...
	if(ps_cali.valid == 1)
		{
			...
			//APS_DBG("get ps value 1111 ps_cali.close = %d ,ps_cali.far_away =%d \n",ps_cali.close,ps_cali.far_away);
			if((ps >ps_cali.close))
			{
				val = 0;  /*close*/
				val_temp = 0;
...
{
     	int i=0 ,err = 0,j = 0;
	u16 data[22],sum=0,data_cali=0,max=0,min=0xffff;
     	printk("enter %s\n",__FUNCTION__);
	sum = 0;
    	mdelay(15);
	for(i = 0;i<22;i++)
	...
	if(i == 22)
	{
		...
		if(data_cali<=800)
		{
			ps_data_cali->close =data_cali+120;
			ps_data_cali->far_away = data_cali+80;
			ps_data_cali->valid =1;
		}
		else{
		  	ps_data_cali->close = 900;
	...
	{
		printk("tmd2772_read_data_for_cali data = fail");
		ps_data_cali->valid = 0;
	 	err=  -1;
	}
