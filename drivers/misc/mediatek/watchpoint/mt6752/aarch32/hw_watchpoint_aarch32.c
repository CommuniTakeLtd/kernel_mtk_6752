...
struct wp_trace_context_t wp_tracer;
 ...
 int my_wp_handler1(unsigned int addr)
 {
     ...
     wp_flag++; 
     pr_debug("[MTK WP] Access my data from an instruction at 0x%x\n" ,addr);
     return 0;
 }
 ...
 int my_wp_handler2(unsigned int addr)
 {
     ...
     //this_cpu = get_cpu();
     pr_debug("[MTK WP] In my_wp_handler2 Access my data from an instruction at 0x%x\n",addr);
     /* trigger exception */
     return 0;
 }
...
//            pr_debug("[MTK WP] cpu %d, SDSR 0x%x \n",i,args);
      }
        else
        {
...
int add_hw_watchpoint(struct wp_event *wp_event)
{
    ...
    pr_debug("[MTK WP] Hotplug disable\n");
    cpu_hotplug_disable();
    pr_debug("[MTK WP] Add watchpoint %d at address 0x%x\n", i, wp_tracer.wp_events[i].virt);
