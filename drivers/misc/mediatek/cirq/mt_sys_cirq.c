...
int mt_cirq_test(void)
{
    ...
    if(mt_cirq_get_pol(cirq_num) != MT_CIRQ_POL_NEG)
        pr_debug( "mt_cirq_set_pol clear test failed!!\n");
    else
        pr_debug( "mt_cirq_set_pol clear test passed!!\n");
    mt_cirq_set_pol(cirq_num, MT_CIRQ_POL_POS);
    if(mt_cirq_get_pol(cirq_num) != MT_CIRQ_POL_POS)
        pr_debug( "mt_cirq_set_pol set test failed!!\n");
    else
        pr_debug( "mt_cirq_set_pol set test passed!!\n");

    /*test sensitivity*/
    mt_cirq_set_sens(cirq_num, MT_EDGE_SENSITIVE);
    if(mt_cirq_get_sens(cirq_num) != MT_EDGE_SENSITIVE)
        pr_debug( "mt_cirq_set_sens clear test failed!!\n");
    else
        pr_debug( "mt_cirq_set_sens clear test passed!!\n");
    mt_cirq_set_sens(cirq_num, MT_LEVEL_SENSITIVE);
    if(mt_cirq_get_sens(cirq_num) != MT_LEVEL_SENSITIVE)
        pr_debug( "mt_cirq_set_sens set test failed!!\n");
    else
        pr_debug( "mt_cirq_set_sens set test passed!!\n");
    ...
    if (mt_cirq_get_mask(cirq_num) != 1)
        ...
        pr_debug( "mt_cirq_mask test passed!!\n");
    mt_cirq_unmask(cirq_num);
    if (mt_cirq_get_mask(cirq_num) != 0)
        pr_debug( "mt_cirq_unmask test failed!!\n");
    else
        pr_debug( "mt_cirq_unmask test passed!!\n");
