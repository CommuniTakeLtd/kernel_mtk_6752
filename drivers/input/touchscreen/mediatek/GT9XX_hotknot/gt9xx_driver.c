...
/// wming add for wakeup by touch @{
#if GTP_GESTURE_WAKEUP
static int s_active_tap_flag = 0;   // 1-default open function; 0-default close.
static int s_active_gesture_flag = 0;   // 1-default open function; 0-default close.
#endif
/// @}
...
#define CMD_WRITE_ACTIVE_TAP        0x40000000
#define CMD_READ_ACTIVE_TAP         0x80000000
#define CMD_WRITE_ACTIVE_GESTURE    0x40000001
#define CMD_READ_ACTIVE_GESTURE     0x80000001
static long gt9xx_wakup_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
   // printk("[wming] gt9xx_wakup_ioctl, cmd = %x, arg = %ld", cmd, arg);
    int ret = 0;
    switch (cmd)
    {
        case CMD_WRITE_ACTIVE_TAP:
            s_active_tap_flag = arg;
            break;
        case CMD_WRITE_ACTIVE_GESTURE:
            s_active_gesture_flag = arg;
            break;
        case CMD_READ_ACTIVE_TAP:
            return s_active_tap_flag;
            break;
        case CMD_READ_ACTIVE_GESTURE:
            return s_active_gesture_flag;
            break;
        default:
            break;
    }
...
struct file_operations gt9xx_wakup_fops = {
    .owner = THIS_MODULE,
    .unlocked_ioctl = gt9xx_wakup_ioctl
};

static struct miscdevice gt9xx_wakup_misc = {
    .minor = MISC_DYNAMIC_MINOR,
    .name = "gt9xx_wakup_dev",
    .fops = &gt9xx_wakup_fops
};
#endif
/// @}
...
//#ifdef VELOCITY_CUSTOM
   ...
   /// wming add for wakeup by touch screen @{
#if GTP_GESTURE_WAKEUP
    ret = misc_register(&gt9xx_wakup_misc);
    if (ret < 0)
    {
        GTP_ERROR("create gt9xx_wakup_misc device failed\n");
    }
#endif
   /// @}
...
static int tpd_i2c_remove(struct i2c_client *client)
{
/// wming add for wakup by touch screen @{
#if GTP_GESTURE_WAKEUP
    misc_deregister(&gt9xx_wakup_misc);
#endif
/// @}
...
#endif
//@fuwanan add for switch gesture --start
#if GTP_GESTURE_WAKEUP
static void switch_gesture(int gesture_id)
{
  // printk("[wming], switch_gesture s_active_tap_flag = %d, s_active_gesture_flag = %d\n", s_active_tap_flag, s_active_gesture_flag);
...
/*
				...
				input_report_key(tpd->dev, KEY_POWER, 1);//@fuwanan add for wake up screen
				input_sync(tpd->dev);
...
*/
		...
		case GESTURE_O:
                      /// wming add for wakeup by touch @{
                          if (s_active_gesture_flag == 0)  break;
                      /// @}
...
#endif
//@fuwanan add for switch gesture --end
static int touch_event_handler(void *unused)
{
   ...
   u8  rqst_data[3] = {(u8)(GTP_REG_RQST >> 8), (u8)(GTP_REG_RQST & 0xFF), 0};
    ...
    do
    {
	    ...
	    ret = gtp_i2c_read(i2c_client_point, doze_buf, 3);
            GTP_DEBUG("Gesture data 0x814B = 0x%02X", doze_buf[2]);
	    GTP_DEBUG("Gesture char :%c", doze_buf[2]);
            if (ret > 0)
            {               
		if(doze_buf[2]!=0){
			GTP_INFO("Go to switch gesture data ");
			//doze_status = DOZE_WAKEUP;
			switch_gesture(doze_buf[2]);
                     	// clear 0x814B
                    	doze_buf[2] = 0x00;
...
#if (defined(GTP_HAVE_TOUCH_KEY)||defined(TPD_HAVE_BUTTON))
