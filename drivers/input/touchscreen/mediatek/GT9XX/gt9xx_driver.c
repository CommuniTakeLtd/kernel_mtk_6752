...
const u16 touch_key_array[] = GTP_KEY_TAB;//TPD_KEYS;
...
static void tpd_eint_interrupt_handler(void)
{
    GTP_INFO("tpd_eint_interrupt_handler");//@fuwanan add 
    TPD_DEBUG_PRINT_INT;
...
static int touch_event_handler(void *unused)
{
   ...
   u8  rqst_data[3] = {(u8)(GTP_REG_RQST >> 8), (u8)(GTP_REG_RQST & 0xFF), 0};
    ...
    do
    {
        ...
        tpd_flag = 0;
        GTP_DEBUG("set_current_state(TASK_RUNNING);");
...
printk("wangkun key_value & (0x01 << i) = %d key_value = %d \n",(key_value & (0x01 << i)),key_value);
