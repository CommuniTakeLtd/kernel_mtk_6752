 ...
 * socket might be less than that of its clients if these break
 * their
 * association with it or if the server socket is closed while
 * clients
 * are still connected to it and there's no way to inform "a
 * polling
 * implementation" that it should let go of a certain wait queue
 *
 * In order to propagate a wake up, a wait_queue_t of the
 * client
 * socket is enqueued on the peer_wait queue of the server
 * socket
 * whose wake function does a wake_up on the ordinary client
 * socket
 * wait queue. This connection is established whenever a
 * write (or
 * poll for write) hit the flow control condition and
 * broken when the
 * association to the server socket is dissolved or after
 * a wake up
 * was relayed.
 */
				      ...
				      void *key)
{
	...
	u = container_of(q, struct unix_sock,
			 peer_wake);
	...
	if
		(!u->peer_wake.private)
		{
			u->peer_wake.private
				=
				other;
			__add_wait_queue(&u_other->peer_wait,
					 &u->peer_wake);
...
static int unix_dgram_peer_wake_me(struct sock *sk, struct sock *other)
{
	...
	if (connected)
		unix_dgram_peer_wake_disconnect(sk,
						other);
